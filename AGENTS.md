# 記帳 App Backend（loco.rs / Rust）

> **文件維護**：每次改完 code（任何行為、endpoint、schema、業務規則改動）都必須同步更新對應 `agent-specs/*.md` 及 `swagger/` OpenAPI；未更新 spec 嘅改動當未完成。
>
> 本專案已由 Rails 8 rewrite 成 **loco.rs (Rust)**。API contract、DB schema、錯誤格式維持 100% 相容；`agent-specs/01`–`03` 仍然有效，只有技術棧／部署（`00`、`04`、`05`）改成 Rust 版本。
...
| [`agent-specs/04-phases.md`](agent-specs/04-phases.md)   | 開發 Phases 0–7（Rails 原版）+ Phase R（loco.rs rewrite）        |
...
```bash
cargo loco db migrate            # 跑 migration
cargo loco start                 # 起 server（預設 port 3000）
cargo test                       # 測試
cargo fmt --all                  # 格式化（改完 code 必跑）
cargo clippy --all-targets -- -D warnings
```

> 修改 Rust code 後必須執行 `cargo fmt --all` 同 `cargo clippy --all-targets`（等同其他專案嘅 typecheck / format 檢查）。
