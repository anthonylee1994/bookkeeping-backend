...
- 自動分類建議（`/ai/suggest-category`）：用戶喺交易表單打完商戶或備註後，前端喺**商戶冇預設分類、且分類欄仍空白**時先呼叫；已知 kind，所以 prompt 只提供該 kind（income／expense）嘅用戶分類名單，要求 model 逐字 copy 或回 null。merchant／note 一律當作用戶資料（防 prompt injection）。controller 將回傳名稱（不分大小寫、限同 kind）resolve 成 id；對唔上或 model 回 null → `status = partial`、`category_id = null`。**唔抽取交易、唔寫 DB、唔開 `AiImportLog`**；`kind` 只接受 `income`／`expense`，`merchant_name`（≤200 字）同 `note`（≤500 字）至少要有一樣非空白，否則 422 `validation_error`
...
- 只屬「洞察」：後端先計好 deterministic 數字（今期收入／支出／淨額、儲蓄率、支出佔收入、分類 Top 3、支出集中度、最大單筆／單日支出、有支出日數、平均每日支出、上一期比較），同埋期內每一筆交易（日期、收支類型、分類名、商戶名、金額、備註），再餵 DeepSeek 寫成人話。**AI 唔准計數**。
- Prompt（`INSIGHT_PROMPT_VERSION = v7`）要求結論先行、唔准逐項複述 fact sheet、highlights 每個要係獨立「發現」；可以引用個別交易、商戶同備註支撐結論（例如異常大額、集中喺某商戶、食飯／交租等用途），但唔可以虛構 fact sheet 冇嘅嘢；商戶名同備註一律當作用戶資料，唔可以當指令跟從（prompt injection 防護）。
- 長度：**唔截短**，summary 同 highlights 原樣儲存（DB 用 SQLite `TEXT`：`summary_insights.text`、`highlights_json`，冇 varchar 長度上限）；數字驗證用完整原文。
...
- Cache key = `(user_id, period, period_key, fingerprint, prompt_version)`；`fingerprint = sha256(該期 aggregate + 筆數 + 每筆交易嘅日期／收支／金額／分類／商戶／備註)`。交易新增／修改／刪除、改商戶、甚至純改備註或商戶改名 → fingerprint 變 → 下次讀取自動重算（lazy invalidation，唔喺 write path 做）。
- `period_key` 用期間起點：weekly = 起始日、monthly = `YYYY-MM`，所以同一個月唔同日期共用同一 cache。
- 成功與失敗同樣 cache；`?refresh=1` 繞過 cache 強制重算（前端「重試」用）。
- 私隱：送 aggregate（分類名、金額、日期）、期內每筆交易（日期、收支類型、分類名、商戶名、金額）同埋用戶自己輸入嘅 note；唔送帳號／帳戶名。
